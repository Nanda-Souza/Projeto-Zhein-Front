...
export default function IdProdutos() {
  ...
  useEffect(() => {
    ...
    const promise = axios.get(
      `${process.env.REACT_APP_API_URL}/produto/${id_do_produto}`,
      config
    );
  ...
  function addInfo(e) {
    ...
    const body = { nome, preco, url };

    console.log({ body });

    const promise = axios.post(
      `${process.env.REACT_APP_API_URL}/carrinho`,
      body,
      config
    );
  ...
  return (
    <Container>
      ...
      <Buttons>
        <button onClick={addInfo}>Adicionar à sacola</button>
      </Buttons>
    </Container>
  );
}
